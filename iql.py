    ...
    def act(self, obss) -> List[int]:
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for i in range(self.num_agents):
            obs = obss[i] # get observation for each agent

            # ε-greedy: random action
            if np.random.rand() < self.epsilon:
                action = random.randrange(self.n_acts[i])
            else:
                # greedy action: argmax_a Q_i(obs, a)
                q_values = [self.q_tables[i][str((obs, a))] for a in range(self.n_acts[i])]
                # if there are many best actions, this line would always choose the first one
                # action = int(np.argmax(q_values)) 
                max_q = max(q_values)
                best_actions = [a for a, q in enumerate(q_values) if q == max_q]
                # if there are multiple best_actions, choose randomly
    ...
    def learn(
        ...
        **IMPLEMENT THIS FUNCTION**
        ...
        for i in range(self.num_agents):
            ...
            q_sa = self.q_tables[i][str((obs, a))]

            # Compute max_a' Q(s', a')
            if done:
                max_next_q = 0.0
            else:
                max_next_q = max(self.q_tables[i][str((n_obs, a_prime))] for a_prime in range(self.n_acts[i]))

            # Q-learning update
            updated_q = q_sa + self.learning_rate * (rew + self.gamma * max_next_q - q_sa)
            self.q_tables[i][str((obs, a))] = updated_q
